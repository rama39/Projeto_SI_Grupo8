...
function desenharMapaVisualizacao() {
  ...
  switch (busca) {
    ...
    case "Gulosa":
      ...
      break;
    case "Custo Uniforme":
      visitadosAtual = visitadosCusto;
      fronteiraAtual = fronteiraCusto;
      break;
  }
...
function desenharComida() {
  ...
  noStroke();
  ellipse(
    comida.x * tamanhoCelula + tamanhoCelula / 2,
    comida.y * tamanhoCelula + tamanhoCelula / 2,
    tamanhoCelula * 0.9,
  );
}
...
function desenharCaminhoFinal() {
  if (caminho.length > 1) {
    ...
    for (let i = 1; i < caminho.length; i++) {
      vertex(
        caminho[i].x * tamanhoCelula + tamanhoCelula / 2,
        caminho[i].y * tamanhoCelula + tamanhoCelula / 2,
      );
    }
    endShape();
    stroke(0, 0, 0, 0);
...
function desenharPlacarELegendas() {
  ...
  text(`Comidas coletadas: ${comidasColetadas}`, 250, 720);
...
function atualizarLogicaMovimentoSuave() {
  if (estado === "MOVENDO" && caminho.length > 1) {
    ...
    agenteReal.x = lerp(
      atual.x * tamanhoCelula + tamanhoCelula / 2,
      proximo.x * tamanhoCelula + tamanhoCelula / 2,
      progressoSuave,
    );
    agenteReal.y = lerp(
      atual.y * tamanhoCelula + tamanhoCelula / 2,
      proximo.y * tamanhoCelula + tamanhoCelula / 2,
      progressoSuave,
    );
